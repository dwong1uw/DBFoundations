...
-- 11/18/21, DWong, Created file, tables, add constraints, add data
-- 11/19/21, DWong, Created code for Q1-Q2
-- 11/20/21, DWong, Created code for Q3-Q9
-- 11/21/21, DWong, Created code for Q10, completed file
--**************************************************************************--
Begin Try
	Use Master;
	If Exists(Select Name From SysDatabases Where Name = 'Assignment06DB_DianaWong')
	 Begin 
	  Alter Database [Assignment06DB_DianaWong] set Single_user With Rollback Immediate;
	  Drop Database Assignment06DB_DianaWong;
	 End
	Create Database Assignment06DB_DianaWong;
End Try
Begin Catch
	Print Error_Number();
End Catch
...
CREATE VIEW vInventoriesByProductsByCategories AS
		...
		FROM vCategories AS c
			...
			JOIN vInventories AS i
				ON p.ProductID = i.ProductID
		ORDER BY c.CategoryName, p.ProductName, InventoryDate, Count;
GO
...
CREATE VIEW vInventoriesForChaiAndChangByEmployees AS
	...
	ORDER BY InventoryDate, c.CategoryName, p.ProductName;
GO
...
CREATE VIEW vEmployeesByManager AS
		...
		FROM vEmployees AS Emp Inner Join Employees Mgr
